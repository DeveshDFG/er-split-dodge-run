...
The repository contains diagnostic probes used during development, including live keybind and `CSPcKeyConfig` probes. They are kept for debugging and research, but are not required for building or using the mod.

For normal play, do not load:

```text
libER.dll
libER_cspc_key_config_probe.dll
key_config_live_bind_probe.dll
other libER probe DLLs
```

Use only `separate_roll_and_sprint.dll`, `separate_roll_and_sprint.ini`, and the deployed HKS package.
...
| `c0000.hks` | Root reference copy of the deployed HKS |
